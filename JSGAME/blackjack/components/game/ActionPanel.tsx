  ...
  if (mode === "betting") {
    return (
      <nav className="action-panel action-panel--betting" aria-label="下注操作">
        <div className="betting-fields">
          <span className="bet-label">{currentBet ? `当前底注 ${currentBet.toLocaleString()} $` : "本局底注"}</span>
          <input className="bet-input" aria-label={currentBet ? "加注金额" : "底注金额"} type="number" min="1" max={currentBet ? Math.max(0, bankroll - Math.max(0, currentBet - committedBet)) : bankroll} step="1" value={bet || ""} placeholder={currentBet ? "输入加注金额" : "输入底注金额"} onChange={(event) => onSetBet(Number(event.target.value) || 0)} disabled={!bettingTurn} />
          <span className="bet-available">可用 {bankroll.toLocaleString()} $</span>
        </div>
        <div className="betting-actions">
          <button className="action action--reuse" type="button" onClick={onUsePreviousBet} disabled={!bettingTurn || !previousBet || previousBet > (currentBet ? bankroll - Math.max(0, currentBet - committedBet) : bankroll)}>使用上把赌资</button>
          {currentBet > 0 && <button className="action action--stand" type="button" onClick={onFold} disabled={!bettingTurn}>退出</button>}
          {currentBet - committedBet > bankroll && <button className="action action--double" type="button" onClick={onAllIn} disabled={!bettingTurn || bankroll <= 0}>All-in {(bankroll + committedBet).toLocaleString()} $</button>}
          {currentBet > 0 && <button className="action action--hit" type="button" onClick={onCall} disabled={!bettingTurn || currentBet - committedBet > bankroll}>跟注 {currentBet.toLocaleString()} $</button>}
          <button className={`action action--double ${currentBet ? "action--raise" : ""}`} type="button" onClick={onConfirmBet} disabled={!bettingTurn || !bet || bet > (currentBet ? bankroll - Math.max(0, currentBet - committedBet) : bankroll)}>{currentBet ? `加注 ${(currentBet + bet).toLocaleString()} $` : "确认底注"}</button>
        </div>
      </nav>
    );
  }
